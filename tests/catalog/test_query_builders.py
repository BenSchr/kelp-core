"""Tests for Unity Catalog SQL query generation (query_builders package)."""
...
class TestTableDescriptionQueries:
    ...
    def test_table_description_view(self):
        ...
        queries = factory.build("catalog.schema.my_view", diff, "view")
    ...
    def test_table_description_materialized_view(self):
        ...
        queries = factory.build("catalog.schema.mv", diff, "materialized_view")
    ...
    def test_table_description_streaming_table(self):
        """Test COMMENT ON statement for streaming tables."""
        factory = UCQueryBuilderFactory()

        diff = TableDiff(table_description="Streaming table description")
        queries = factory.build("catalog.schema.st", diff, "streaming_table")

        assert len(queries) == 1
        assert queries[0] == "COMMENT ON TABLE catalog.schema.st IS 'Streaming table description'"
...
class TestConstraintQueries:
    ...
    def test_add_foreign_key(self):
        ...
        queries = factory.build("catalog.schema.orders", diff, "managed")
